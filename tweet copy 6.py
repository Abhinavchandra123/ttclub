...
def search_tweets(driver, topic_names, from_account, minimum_replies, minimum_likes, minimum_retweets, include_replies, include_links,number_of_tweets):
    ...
    for topic_name in topic_names:
        ...
        for article in articles:
            like_elements.extend(wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, '[data-testid="like"]')), article))
            engagement_elements.extend(wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, '[aria-label*="views"]')), article))
            views_elements.extend(wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'div[aria-label*="replies"][aria-label*="reposts"][aria-label*="likes"][aria-label*="bookmarks"][aria-label*="views"]')), article))
            tweets.extend(wait.until(EC.presence_of_all_elements_located((By.XPATH, '//div[@data-testid="tweetText"]')), article))
            usernames.extend(wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, '[data-testid="User-Name"]')), article))
            hrefs.extend(wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "[data-testid=User-Name] a[role=link][href*=status]")), article))

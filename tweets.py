...
def get_tweet_parameters(option):
    while True:
        try:
            ...
            if 1 <= number_of_tweets <= 50:  # Check if within range (1 to 20)
                break
            else:
        ...
        try:
            ...
            try:
                while attempt_count < max_attempts:
                    ...
                    for username, like_element, engagement_element, href_element, reply, repost,timee in zip(
                        ...
                        for article in new_articles:
                            ...
                            if href == article_href:
                                ...
                                try:
                                    tweet_username=article.find_element(By.CSS_SELECTOR, '[data-testid="User-Name"]')
                                    user_full_name = (tweet_username.find_element(By.XPATH, "..").text.strip().rstrip("."))
                                    user = re.search(r'@(\w+)', user_full_name).group()
                    ...
                    if tweet_count >= number_of_tweets:
                        break
                    driver.execute_script("arguments[0].scrollIntoView();", new_articles[-1])#scroll to last element
                    #when scroll to last element dom will still have 3 to 4 old post so we scroll again
                    #here one tweet should have around 500 to 700 px height
                    #so we can add 2 or 3 tweet height to scroll  edit below
                    driver.execute_script("window.scrollBy(0, 2000);")
                    # new_articles[-1].send_keys(Keys.END)
...
def main():
    ...
    while True:
        ...
        if option == "1":
            ...
            options.add_argument('--no-sandbox')  # Needed for Linux servers
            options.add_argument('--disable-dev-shm-usage')  # Needed for Linux servers
            ...
            #needed for server when configuring chrome driver adjust path (most likely path)
